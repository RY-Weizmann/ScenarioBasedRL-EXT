...
class RoboticNavigation( gym.Env ):
	...
	"""
	# General idea to provide the informations to PPO, not used now, the idea is to provide only a flag
	# to the PPO algorithm for the vioaltion of one of the rules:
	#
	# info["rules_value"] = self.get_rules_penalty( state, reward, action, done, info )
	#
	def get_rules_penalty( self, state, reward, action, done, info ):

		#
		raise NotImplementedError()

		# return if rules are inactive
		if not self.rules_active: return 0

		# Return the penaly only on terminal states (i.e., done == True)
